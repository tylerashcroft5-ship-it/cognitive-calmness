...
import { CAL_CONFIGURED, calUrl } from "@/lib/booking";
...
/* ---------- BookButton: Cal.com when configured, enquiry form until then ---------- */
export function BookButton({ slug, children, className = "btn btn--primary" }) {
  if (CAL_CONFIGURED) {
    return (
      <a href={calUrl(slug)} target="_blank" rel="noreferrer" className={className}>
        {children} <span className="arrow">→</span>
      </a>
    );
  }
  return (
    <Link href="/book#enquire" className={className}>
      {children} <span className="arrow">→</span>
    </Link>
  );
}
