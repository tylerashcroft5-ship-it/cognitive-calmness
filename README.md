...
The site is wired for **Cal.com** (free tier is enough):
1. Client creates an account at cal.com **on her phone or desktop** — the Cal.com mobile app gives her instant notifications and lets her manage availability from her phone.
2. Create event types whose URLs match the slugs in `lib/booking.js` (`wellbeing-30`, `wellbeing-60`, `cbt-session`, `one-off`, `supervision`) — or edit the slugs to match whatever she creates.
3. In `lib/booking.js`, set `CAL_USERNAME` to her Cal.com username and flip `CAL_CONFIGURED` to `true`.

Until that flag is flipped, every Book button gracefully routes to the enquiry form instead, and the booking page shows a "coming soon" note — so the site can launch before the calendar is ready.
